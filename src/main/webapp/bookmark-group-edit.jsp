...
<% } else { %>
    <form action="/edit-bookmark-group" method="post">
        <input type="hidden" id="id" name="id" value="<%=bookmarkGroup.getId()%>" />
        <table id="default-table">
            <tr>
                <th><label for="name">북마크 이름</label></th>
                <td><input type="text" id="name" name="name" value="<%=bookmarkGroup.getName()%>"/></td>
            </tr>
            <tr>
                <th><label for="order">순서</label></th>
                <td><input type="number" id="order" name="order" value="<%=bookmarkGroup.getOrder()%>"/></td>
            </tr>
            <tr>
